        ...
        <form id="loginForm">
            <div class="mb-3">
                ...
                <div class="input-group input-group-lg">
                    ...
                    <input type="email" class="form-control fs-6" id="email" placeholder="admin@jankivilla.com" required>
                </div>
            </div>
        ...
        $('#loginForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#messageBox').removeClass('d-none').html('<div class="alert alert-warning border-0 bg-warning bg-opacity-10 text-warning-emphasis"><i class="fas fa-envelope-open-text me-2"></i>Waiting for Admin approval...</div>');
                },
                error: function(xhr) {
                    ...
                    $('#messageBox').removeClass('d-none').html('<div class="alert alert-danger border-0 bg-danger bg-opacity-10 text-danger"><i class="fas fa-exclamation-circle me-2"></i>' + errorMessage + '</div>');
                }
            });
        });
        ...
        window.Echo.channel('login-status.' + currentLoginSessionId)
            .listen('.LoginApproved', (data) => {
                ...
                if (data.status === 'approved') {
                    $('#messageBox').html('<div class="alert alert-success border-0 bg-success bg-opacity-10 text-success"><i class="fas fa-check-circle me-2"></i>Access Granted! Redirecting...</div>');
                    ...
                    $('#messageBox').html('<div class="alert alert-danger border-0 bg-danger bg-opacity-10 text-danger"><i class="fas fa-times-circle me-2"></i>Access Denied by Administrator.</div>');
                    $('#loginBtn').html('Secure Login <i class="fas fa-arrow-right ms-2"></i>').prop('disabled', false);
                }
