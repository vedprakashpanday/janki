    ...
    <script>
        ...
        function renderTopButtons(perms) {
            ...
            if (perms.can_add_direct) {
                btnHtml +=
                    `<button class="btn btn-success me-2" onclick="openAddModal('active')"><i class="fa fa-plus"></i> Add Incentive</button>`;
            } else if (perms.can_add_request) {
                btnHtml +=
                    `<button class="btn btn-warning me-2" onclick="openAddModal('pending')"><i class="fa fa-paper-plane"></i> Request Incentive</button>`;
            }

            if (perms.can_export) {
                btnHtml +=
                    `<button class="btn btn-info me-2 text-white" onclick="exportExcel()"><i class="fa fa-file-excel"></i> Export</button>`;
            }

            if (perms.can_print) {
                let portal = "{{ request()->segment(1) }}";
                let currentToken = localStorage.getItem('token') || localStorage.getItem('admin_token') || localStorage
                    .getItem('emp_token') || '';
                ...
                btnHtml +=
                    `<button class="btn btn-secondary" onclick="window.open('${printUrl}', '_blank')"><i class="fa fa-print"></i> Print</button>`;
            }
    ...
    <button class="btn btn-sm btn-outline-danger temp-delete-btn" data-id="${data || row.id}" title="Temporary Delete"><i class="fa fa-trash"></i></button>
        ...
        $(document).on('click', '.view-btn', function() {
            ...
            $.get(`/api/v1/incentives/${id}`, function(res) {
                ...
                let badge = d.incentive_status === 'active' ? 'bg-success' : (d.incentive_status ===
                    'pending' ? 'bg-warning' : 'bg-danger');
                $('#view_status').attr('class', 'badge ' + badge).text(d.incentive_status.toUpperCase());

                let rows = '';
                res.history.forEach(h => {
                    let hBadge = h.incentive_status === 'active' ? 'text-success' : (h
                        .incentive_status === 'pending' ? 'text-warning' : 'text-danger');
                    rows += `<tr>
        ...
        function initCascadingSelect2() {
            ...
            $('#company_id').select2({
                ...
                $('#branch_id, #department_id, #designation_id, #emp_ids').val(null).trigger('change');
            });
            ...
            $('#branch_id').select2({
                ...
                $('#department_id, #designation_id, #emp_ids').val(null).trigger('change');
            });
            ...
            $('#department_id').select2({
                ...
                $('#designation_id, #emp_ids').val(null).trigger('change');
            });
            ...
            $('#emp_ids').select2({
                ...
                ajax: {
                    ...
                    processResults: data => ({
                        results: data.data.map(item => ({
                            id: item.member_id,
                            text: `${item.full_name} (${item.member_id})`
                        }))
                    })
                }
