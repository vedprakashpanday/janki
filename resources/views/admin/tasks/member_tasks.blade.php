...
@section('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="fw-bold text-dark mb-0"><i class="fas fa-users text-warning me-2"></i> Associate Tasks</h4>
                <small class="text-muted">Manage, track, and complete field goals.</small>
            </div>
        ...
        <div class="row mb-3">
            <div class="col-md-5 col-lg-4">
                <div class="input-group shadow-sm border border-secondary border-opacity-25 rounded-3 overflow-hidden">
                    <span class="input-group-text bg-white border-0 text-warning"><i class="fas fa-search"></i></span>
                    <input type="text" id="liveSearchTasks" class="form-control border-0 shadow-none ps-1"
                        placeholder="Search by Associate or Task Title...">
                </div>
            </div>
        </div>

        <div class="alert alert-danger d-flex justify-content-between align-items-center mb-3 shadow-sm py-2 px-3 secured-item"
            data-permission="task_mem_delete" id="bulkDeleteZone" style="display: none !important;">
            <div>
                ...
                <div class="modal-body p-4 bg-white">
                    <form id="assignTaskForm">
                                ...
                                <div class="col-md-3 position-relative">
                                    <div class="d-flex justify-content-between align-items-end mb-1">
                                        <label class="small fw-bold text-dark mb-0"
                                            id="targetDeptLabel">Departments</label>
                                        <div class="filter-actions"><a class="text-primary btn-all"
                        ...
                        <div class="mb-4 position-relative">
                            <div class="d-flex justify-content-between align-items-end mb-1">
                                <label class="small fw-bold text-danger mb-0" id="targetUserLabel">Target Associates
                                    *</label>
                                <div class="filter-actions"><a class="text-primary btn-all"
                ...
                <div class="modal-body p-0 d-flex flex-column" style="background: #eef2f5; height: 80vh;">
                    ...
                    <div class="flex-grow-1 overflow-auto" id="taskModalScrollArea">
                        <div class="p-3 bg-white border-bottom shadow-sm">
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <p class="text-muted small mb-1"><i class="fas fa-calendar-alt text-primary me-1"></i>
                                        Due Date</p>
                        ...
                        <div class="p-3 pb-0">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <a href="#" id="toggleDescBtn"
                                    class="small fw-bold text-primary text-decoration-none" style="display: none;">Read
                                    More <i class="fas fa-chevron-down"></i></a>
                            </div>
                            <div class="bg-white p-3 rounded border shadow-sm small text-dark desc-collapse"
                                id="detailTaskDesc" style="white-space: pre-wrap;">
                                Loading description...
                            </div>
                        </div>
                        ...
                        <div class="p-3 mt-2"
                            ...
                            <div id="detailTimeline" class="mb-2 p-2 rounded"
                                ...
                                <div class="text-center text-muted small py-3"><i class="fas fa-spinner fa-spin"></i>
                    ...
                    <div class="p-3 border-top bg-white z-1 shadow-lg mt-auto w-100">
                        <form id="taskReplyForm">
                            ...
                            <div class="row align-items-center g-2 mb-2">
                                ...
                                <div class="col-md-7">
                                    <input type="file" name="attachments[]" id="chatFileInput"
                            ...
                            <div class="input-group shadow-sm position-relative">
                                <textarea name="message_or_remark" id="chatMessageInput" class="form-control"
                                    placeholder="Type your message here... @ to tag" rows="1" required
                                    style="resize: none; overflow-y: hidden; max-height: 120px;"></textarea>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // 🔥 1. Inject UI Button for Associate Tasks 🔥
            let btnHtml = `
            <button class="btn btn-warning text-dark fw-bold shadow-sm open-assign-modal secured-item" data-permission="task_mem_add_direct">
            ...
            $(document).on('click', '.open-assign-modal', function() {
                $('#hiddenAssigneeType').val('App\\Models\\Member');

                $('#modalTitle').html(
                    `<i class="fas fa-users text-warning me-2"></i> Bulk Assign to Associates`);
                $('#targetUserLabel').html(
                `Target Associates / Members <span class="text-danger">*</span>`);
                $('#targetDeptLabel').text('Associate Departments');
                $('#targetDesigLabel').text('Associate Designations');
            ...
            $(document).on('click', '.btn-clear', function() {
                ...
                if (target === 'companySelect') $('#branchSelect, #deptSelect, #desigSelect, #userSelect')
                    .empty().val(null).trigger('change');
                else if (target === 'branchSelect') $('#deptSelect, #desigSelect, #userSelect').empty().val(
                    null).trigger('change');
                else if (target === 'deptSelect') $('#desigSelect, #userSelect').empty().val(null).trigger(
                    'change');
                else if (target === 'desigSelect') $('#userSelect').empty().val(null).trigger('change');
            });
            ...
            $('#companySelect').select2({
                ...
                dropdownParent: $('#companySelect').parent(),
                ajax: {
            ...
            $('#branchSelect').select2({
                ...
                ajax: {
                    ...
                    processResults: function(data, params) {
                        ...
                        if (params.term && (params.term.toLowerCase().includes('hea') || params.term
                                .toLowerCase().includes('off'))) {
                            selectedCompanies.forEach(comp => {
                                if (comp.id) results.push({
                                    id: 'HO_' + comp.id,
                                    text: 'Head Office (' + comp.text + ')'
                                });
                            });
                        }
                        let mapped = $.map(data.data, function(item) {
                            return {
                                id: item.id,
            ...
            $('#deptSelect').select2({
                ...
                width: '100%',
                minimumInputLength: 3,
                dropdownParent: $('#deptSelect').parent(),
                ajax: {
            ...
            $('#desigSelect').select2({
                ...
                width: '100%',
                minimumInputLength: 3,
                dropdownParent: $('#desigSelect').parent(),
                ajax: {
            ...
            $('#userSelect').select2({
                ...
                ajax: {
                    url: apiPrefix + '/task-dependencies/members',
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
            ...
            $(document).on('click', '.remove-task-row', function() {
                $(this).closest('.task-row').slideUp(200, function() {
                    $(this).remove();
                });
            });
            ...
            $('#assignTaskForm').on('submit', function(e) {
                ...
                if (!globalAssignees || globalAssignees.length === 0) {
                    Swal.fire('Wait!', 'Please select at least one person from the Target list.',
                    'warning');
                    return;
                }
                ...
                Object.values(requestGroups).forEach(group => {
                    ...
                    group.tasks.forEach((t, i) => {
                        ...
                        if (t.tracking_module_id) fd.append(
                            `tasks[${i}][tracking_module_id]`, t.tracking_module_id);
            ...
            function renderTasks() {
                ...
                let apiUrl = apiPrefix + '/tasks?type=App\\Models\\Member';
                    ...
                    success: function(res) {
                        try {
                            if (!res || !res.data || !Array.isArray(res.data) || res.data.length ===
                                0) {
                                $('#taskBoard').html(
                                    '<div class="col-12 text-center text-muted py-5"><i class="fas fa-check-circle fa-3x mb-3 text-success"></i><h5>No active tasks found today!</h5></div>'
                                    );
                                return;
                            }
                        ...
                        <div class="col-12 mb-3 text-end">
                            ...
                            res.data.forEach(task => {
                                ...
                                if (task.assignee) {
                                    assigneeName = task.assignee.member_name || task.assignee
                                        .full_name || task.assignee.name || 'Unknown User';
                            ...
                            sortedEmpIds.forEach(empId => {
                                ...
                                let isDeletePermitted = window.userGodMode || (window
                                    .userPerms && window.userPerms.includes(
                                        'task_mem_delete'));
                                let empSelectAllHtml = isDeletePermitted ?
                                ...
                                emp.tasks.forEach(task => {
                                    ...
                                    if (task.progress_logs && task.progress_logs
                                        ...
                                        let actorName = lastLog.actor ? (lastLog.actor
                                                .member_name || lastLog.actor
                                                .full_name || lastLog.actor.name) :
                                            'System';
                                    ...
                                    let checkboxHtml = isDeletePermitted ?
                                ...
                                <div class="row align-items-center border-bottom p-3 m-0 task-row-hover task-row-item ${unreadRowClass}">
                                    ...
                                    <div class="col-12 col-md-3 col-lg-3 text-md-end d-flex gap-1 justify-content-md-end">
                                        ...
                                        <button class="btn btn-sm btn-light border text-success shadow-sm edit-task-btn secured-item" data-permission="task_mem_edit" data-id="${task.id}" title="Edit Task"><i class="fas fa-edit"></i></button>
                                        <button class="btn btn-sm btn-light border text-danger shadow-sm delete-task-btn secured-item" data-permission="task_mem_delete" data-id="${task.id}" title="Delete Task"><i class="fas fa-trash-alt"></i></button>
                                    </div>
                                </div>`;
                                });
                        ...
                        .applyPermissions();
                            ...
                            $('#taskBoard').html(
                                `<div class="col-12 text-center py-5 text-danger"><h5>Frontend Rendering Crashed!</h5><p>${jsError.message}</p></div>`
                                );
                        }
                    },
                    error: function(xhr, status, error) {
                        $('#taskBoard').html(
                            `<div class="col-12 text-center py-5 text-danger"><i class="fas fa-wifi fa-3x mb-3"></i><h5>Failed to load tasks!</h5><button class="btn btn-sm btn-outline-secondary mt-2" onclick="location.reload()">Refresh Page</button></div>`
                            );
                    }
                });
            }
            ...
            function initTopFilters() {
                let ajaxConfig = (urlExt, dependFunc) => {
                    return {
                        url: apiPrefix + urlExt,
                        dataType: 'json',
                        delay: 250,
                        data: function(p) {
                            let d = {
                                search: p.term,
                                assignee_type: 'App\\Models\\Member'
                            };
                            if (dependFunc) Object.assign(d, dependFunc());
                            return d;
                        },
                        processResults: function(data) {
                            return {
                                results: $.map(data.data, function(i) {
                                    return {
                                        id: i.id,
                                        text: i.name || i.company_name || i.branch_name || i
                                            .department_name || i.designation_name
                                    };
                                })
                            };
                        }
                ...
                $('#filterCompany').select2({
                    ...
                    ajax: ajaxConfig('/task-dependencies/companies')
                });
                        ...
                        processResults: function(data, params) {
                            ...
                            if (params.term && (params.term.toLowerCase().includes('hea') || params.term
                                ...
                                if (selectedCompany && selectedCompany.length > 0) {
                                    ...
                                    results.push({
                                        id: 'HO_ALL',
                                        text: 'Head Office (All)'
                                    });
                                }
                            }

                            let mapped = $.map(data.data, function(i) {
                                return {
                                    id: i.id,
                                    text: i.branch_name
                                };
                ...
                $('#filterDepartment').select2({
                    ...
                    ajax: ajaxConfig('/task-dependencies/departments', () => ({
                        company_ids: $('#filterCompany').val(),
                ...
                $('#filterDesignation').select2({
                    ...
                    ajax: ajaxConfig('/task-dependencies/designations', () => ({
                        company_ids: $('#filterCompany').val(),
                        branch_ids: $('#filterBranch').val(),
                    ...
                    $('.emp-card-wrapper').each(function() {
                        ...
                        if (bId) {
                            if (String(bId).startsWith('HO_')) {
                                if (cardB !== '' && cardB !== 'null' && cardB !== '0') match =
                                false;
                            } else {
            ...
            $(document).on('change', '#chatFileInput', function(e) {
                ...
                if (files.length > 0) {
                    ...
                    Array.from(files).forEach((file, index) => {
                        ...
                        let html =
                            `<div class="position-relative border rounded shadow-sm d-flex align-items-center justify-content-center bg-light" style="width: 50px; height: 50px; overflow: hidden;">`;
                        if (file.type.startsWith('image/')) html +=
                            `<img src="${url}" class="img-fluid" style="object-fit: cover; width:100%; height:100%;">`;
                        else if (file.type.startsWith('video/')) html +=
                            `<i class="fas fa-file-video fa-2x text-primary"></i>`;
                        else if (file.type === 'application/pdf') html +=
                            `<i class="fas fa-file-pdf fa-2x text-danger"></i>`;
                        else html += `<i class="fas fa-file fa-2x text-secondary"></i>`;
                        html += `</div>`;
            ...
            $('#taskDetailsModal').on('shown.bs.modal', function() {
                ...
                if (descEl) {
                    if (descEl.scrollHeight > 80) {
                        $('#toggleDescBtn').removeClass('d-none').show().html(
            ...
            function renderTimelineToModal(task) {
                ...
                if (task.progress_logs && task.progress_logs.length > 0) {
                    ...
                    sortedLogs.forEach(log => {
                        let deletedFor = log.deleted_for ? (typeof log.deleted_for === 'string' ? JSON
                            .parse(log.deleted_for) : log.deleted_for) : [];
                        if (deletedFor.includes(myUserIdentifier)) return;

                        let isSystem = !log.actor_id || log.actor_id === 0 || log.actor_type === null;
                        let actorName = log.actor ? (log.actor.member_name || log.actor.full_name || log
                        ...
                        let logDate = logDateObj.toLocaleString('en-GB', {
                            day: '2-digit',
                            month: 'short',
                            year: 'numeric',
                            hour: '2-digit',
                        ...
                        if (log.is_deleted) {
                            if (isSuperUser) {
                                // 🔥 FIX: Strikethrough removed as per request 🔥
                                messageContent =
                                    `<span class="badge bg-danger mb-2 opacity-75"><i class="fas fa-trash-alt me-1"></i>Deleted</span><br><span class="text-muted" style="white-space:pre-wrap;">${messageContent}</span>`;
                                if (log.is_edited) messageContent +=
                                    ` <small class="text-muted fst-italic" style="font-size:10px;">(edited)</small>`;
                            } else {
                                messageContent =
                                    `<span class="text-muted fst-italic"><i class="fas fa-ban me-1"></i> This message was deleted</span>`;
                                attachmentsHtml = '';
                            }
                        } else {
                            if (log.is_edited) messageContent +=
                            ...
                            let deleteEveryoneOption = (isMe || isSuperUser) ?
                                `<li><a class="dropdown-item delete-chat-btn" href="#" data-id="${log.id}" data-type="for_everyone"><i class="fas fa-trash-alt me-2 text-danger"></i> Delete for Everyone</a></li>` :
                                '';
                            ...
                            if (isMe) {
                                if (isSuperUser || diffMinutes <= 5) {
                                    editOption =
                                        `<li><a class="dropdown-item edit-chat-btn" href="#" data-id="${log.id}" data-msg="${log.message_or_remark.replace(/"/g, '&quot;')}"><i class="fas fa-pen me-2 text-primary"></i> Edit</a></li>`;
                                }
                            }
            ...
            $(document).on('click', '.view-task-btn', function() {
                ...
                if (taskBadge.length && !taskBadge.hasClass('d-none')) {
                    ...
                    if (empId) {
                        ...
                        if (empBadge.length) {
                            ...
                            if (empCount === 0) empBadge.removeClass('blink-anim').addClass('d-none');
                            else empBadge.text(empCount + ' New Msg');
                        }
                    }
                }
                ...
                $.get(apiPrefix + '/tasks/' + taskId, function(res) {
                    ...
                    $('#detailTaskPriority').text('Priority: ' + task.priority).removeClass()
                        .addClass('badge bg-' + (task.priority === 'Urgent' ? 'danger' : (task
                            .priority === 'High' ? 'warning' : 'info')));
                    $('#detailTaskStatus').text('Status: ' + task.status).removeClass().addClass(
                        'badge bg-' + (task.status === 'Completed' ? 'success' : (task
                            .status === 'In-Progress' ? 'primary' : 'warning')));

                    let dDue = task.due_datetime ? new Date(task.due_datetime) : null;
                    $('#detailTaskDue').text(dDue ? dDue.toLocaleString('en-GB', {
                        day: '2-digit',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    let assigneeName = task.assignee ? (task.assignee.member_name || task.assignee
                        .full_name) : 'Unknown';
                    $('#detailTaskAssignee').text(assigneeName);

                    $('#detailTaskDesc').text(task.description || 'No description provided.')
                        .addClass('desc-collapse').removeClass('expanded');
                    $('#toggleDescBtn').hide();
                    setTimeout(() => {
                        let descEl = document.getElementById('detailTaskDesc');
                        if (descEl && descEl.scrollHeight > 100) {
                            $('#toggleDescBtn').show().html(
                                'Read More <i class="fas fa-chevron-down"></i>');
                        } else {
                            $('#toggleDescBtn').hide();
                            $('#detailTaskDesc').removeClass('desc-collapse');
                    ...
                    if (task.attachments && task.attachments.length > 0) {
                        task.attachments.forEach(file => {
                            if (file.task_progress_log_id == null) {
                                ...
                                attachmentsHtml +=
                                    `<a href="/${file.file_path}" target="_blank" class="badge bg-light text-primary border text-decoration-none py-2 px-3 shadow-sm"><i class="fas fa-file-download me-1"></i> ${file.file_name}</a>`;
                            }
                        });
                    }
                    if (hasInitialFiles) {
                            ...
                            e) => {
                                $.get(apiPrefix + '/tasks/' + task.id, function(freshRes) {
                                    ...
                                    if ($('#taskDetailsModal').hasClass('show') && $(
                                            '#replyTaskId').val() == task.id) {
                                        if (freshRes.data.progress_logs && freshRes.data
                                            .progress_logs.length > 0) {
                                            localStorage.setItem('task_read_' + task.id,
                                                freshRes.data.progress_logs[0].id);
                                        }
                                    } else {
            ...
            $(document).on('click', '.edit-chat-btn', function(e) {
                ...
                $('#taskDetailsModal').removeAttr('tabindex');
                ...
                Swal.fire({
                    ...
                    didClose: () => {
                        $('#taskDetailsModal').attr('tabindex', '-1');
                    },
                    ...
                    preConfirm: (newMsg) => {
                        ...
                        let socketId = (typeof window.Echo !== 'undefined' && window.Echo
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                if (res.success) {
                                    Swal.close();
                                    let taskId = $('#replyTaskId').val();
                                    $.get(apiPrefix + '/tasks/' + taskId, function(
                                        freshRes) {
                                        renderTimelineToModal(freshRes.data);
                                    });
                                } else {
                            ...
                            error: function(xhr) {
                                Swal.fire('Error', xhr.responseJSON?.message ||
                                    'Action failed. Check console.', 'error');
                            }
                        });
                    }
                });
            ...
            $('#taskReplyForm').on('submit', function(e) {
                ...
                btn.html('<i class="fas fa-spinner fa-spin"></i>').prop('disabled', true);
                let socketId = (typeof window.Echo !== 'undefined' && window.Echo.socketId()) ? window.Echo
                    .socketId() : '';
                    ...
                    success: function(res) {
                        ...
                        $.get(apiPrefix + '/tasks/' + taskId, function(freshRes) {
                            ...
                            $('#detailTaskStatus').text('Status: ' + freshTask.status)
                                .removeClass().addClass('badge bg-' + (freshTask
                                    .status === 'Completed' ? 'success' : (freshTask
