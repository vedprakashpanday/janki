...
@section('content')
    ...
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <!-- 🔥 ADD BUTTON (ID ADDED FOR RBAC) 🔥 -->
            <button type="button" id="addDesigBtn" class="btn text-white px-3 py-2 shadow-sm"
                style="background-color: var(--brand-primary); display: none;" onclick="openModal('add')">
        ...
        <div class="d-flex d-md-none gap-2 mb-3 flex-column">
            <div class="d-flex gap-2">
                ...
                <button type="button" class="btn text-white shadow-sm px-3"
                    style="background-color: #10b981; display: none;" id="mobileExcelBtn"><i
...
@push('scripts')
    ...
    <script>
        ...
        window.moduleBase = 'member_designation'; // Module Base Code
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            // ==========================================
            let hasAdd = window.userGodMode || window.userPerms.includes(window.moduleBase + '_add') || window
                .userPerms.includes(window.moduleBase + '_add_direct') || window.userPerms.includes(window
                    .moduleBase + '_add_request');
            if (hasAdd) $('#addDesigBtn').show();
            ...
            let table = $('#desigTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '/api/v1/member-designations',
                    data: function(d) {
                    ...
                    data.forEach(d => {
                        ...
                        html += `<div class="mobile-item">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <div><h6 class="fw-bold text-dark mb-1">${d.designation_name}</h6><span class="fw-bold text-primary small">${d.designation_code}</span></div>
                                ${statusHtml}
                            </div>
                            <div class="small text-muted mb-3"><span class="comm-badge">Commission: ${d.commission_percentage}%</span></div>
                            <div class="border-top pt-2 d-flex gap-2">${actionBtns}</div>
                        </div>`;
                    });
                }
            ...
            window.openModal = function(type, id = null) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (type === 'edit') {
                            $.get({
                                url: `/api/v1/member-designations/${id}`,
                                success: function(res) {
                                    let d = res.data;
                                    $('#edit_id').val(d.id);
                                    if (d.branch) {
            ...
            $(document).on('click', '.delete-btn', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `/api/v1/member-designations/${$(this).data('id')}`,
                            type: 'DELETE',
                            success: function(res) {
                                table.ajax.reload(null, false);
