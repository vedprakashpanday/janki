...
@section('content')
    ...
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold mb-0" style="color: var(--sidebar-bg);">Letterhead Directory (All Time)</h4>
            <div>
                <!-- 🛡️ Add Permission (Direct or Request) -->
                <button type="button" class="btn text-white px-3 py-2 shadow-sm secured-item" data-permission="letterhead_dir_add_direct,letterhead_dir_add_request"
        ...
        <div class="d-flex d-md-none gap-2 mb-3">
            ...
            <button type="button" class="btn text-white shadow-sm secured-item" data-permission="letterhead_dir_export" style="background-color: #10b981;"
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (!isAdmin) {
                            ...
                            // CASE: Master HO (company null, branch null) - Everything remains open
                        }
                    }
                });
            }
            ...
            // === 4. DATATABLE (DIRECTORY VIEW) ===
            table = $('#lhTable').DataTable({
                ajax: { 
                    url: '/api/v1/letterheads', // Directory data
                ...
                buttons: [{
                    ...
                    attr: { 'data-permission': 'letterhead_dir_export' }
                }],
                ...
                columns: [
                    ...
                    { 
                        ...
                        render: function(row) {
                            let btns = `<button class="btn btn-sm btn-light text-success me-1 print-btn secured-item" data-permission="letterhead_dir_print" data-id="${row.id}"><i class="fas fa-print"></i></button>`;
                            
                            btns += `<button class="btn btn-sm btn-light text-primary me-1 edit-btn secured-item" data-permission="letterhead_dir_edit" data-id="${row.id}"><i class="fas fa-edit"></i></button>`;
                            
                            btns += `<button class="btn btn-sm btn-light text-danger me-1 delete-btn secured-item" data-permission="letterhead_dir_delete" data-id="${row.id}"><i class="fas fa-trash-alt"></i></button>`;
                            // NAYA: Send Button
                            ...
                            if(row.status === 'pending') {
                                btns += `<button class="btn btn-sm btn-success me-1 approve-btn secured-item" data-permission="letterhead_dir_appr" data-id="${row.id}" title="Approve"><i class="fas fa-check-circle"></i></button>`;
                                btns += `<button class="btn btn-sm btn-danger me-1 reject-btn secured-item" data-permission="letterhead_dir_rej" data-id="${row.id}" title="Reject"><i class="fas fa-times-circle"></i></button>`;
                            }
                            return btns;
                        }
                    }
                    ...
                    html += `
                        <div class="mobile-item lh-card">
                            ...
                            <div class="pt-2 border-top d-flex flex-wrap gap-2">
                                <button class="btn btn-sm btn-light text-success fw-bold flex-fill print-btn secured-item" data-permission="letterhead_dir_print" data-id="${item.id}"><i class="fas fa-print"></i></button>
                                // NAYA: Send Button
                                ...
                                <button class="btn btn-sm btn-light text-primary fw-bold flex-fill edit-btn secured-item" data-permission="letterhead_dir_edit" data-id="${item.id}"><i class="fas fa-edit"></i></button>
                                <button class="btn btn-sm btn-light text-danger fw-bold flex-fill delete-btn secured-item" data-permission="letterhead_dir_delete" data-id="${item.id}"><i class="fas fa-trash"></i></button>
                            </div>
                        </div>`;
                });
