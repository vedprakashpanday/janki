    ...
    <div class="container-fluid p-0">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted small mb-0 d-none d-md-block">Manage Check-in / Check-out boundaries for Companies &
                    Branches.</p>
            </div>
        ...
        <div class="row g-3 d-md-none" id="mobileCardsContainer">
        </div>
    </div>
                ...
                <form id="timeWindowForm">
                    <div class="modal-body bg-light p-4">
                        ...
                        <div class="row g-3 mb-4">
                                ...
                                <div class="p-3 border rounded bg-white shadow-sm">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-6">
                                            ...
                                            <input type="time" class="form-control form-control-sm" id="login_start"
                                                required>
                                        </div>
                                        <div class="col-6">
                                            <label class="small text-muted fw-bold">End Time</label>
                                            <input type="time" class="form-control form-control-sm" id="login_end"
                                                required>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="p-3 border rounded bg-white shadow-sm">
                                    <h6 class="fw-bold text-danger mb-3"><i class="fas fa-sign-out-alt me-1"></i> Logout
                                        Window</h6>
                                    <div class="row g-2">
                                        <div class="col-6">
                                            <label class="small text-muted fw-bold">Start Time</label>
                                            <input type="time" class="form-control form-control-sm" id="logout_start"
                                                required>
                                        </div>
                                        <div class="col-6">
                                            <label class="small text-muted fw-bold">End Time</label>
                                            <input type="time" class="form-control form-control-sm" id="logout_end"
                                                required>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div>
                            <label class="fw-bold text-dark">Minimum Working Hours</label>
                            <div class="input-group input-group-sm w-50">
                                <input type="number" class="form-control" id="min_working_hours" value="8.25"
                                    step="0.25" min="1" required>
                                <span class="input-group-text bg-white text-muted">Hrs</span>
                            </div>
                        </div>
                    </div>
                ...
                <form id="editTimeWindowForm">
                    <div class="modal-body p-4 bg-light">
                        ...
                        <div>
                            ...
                            <input type="number" class="form-control form-control-sm border-primary" id="edit_min_hrs"
                                step="0.25" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-6">
                            <div class="p-3 border rounded bg-success bg-opacity-10 border-success border-opacity-25">
                                ...
                                <h6 class="fw-bold mb-0 text-dark" id="view_login_window">--:-- to --:--</h6>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="p-3 border rounded bg-danger bg-opacity-10 border-danger border-opacity-25">
                                <div class="small text-danger fw-bold mb-1"><i class="fas fa-sign-out-alt"></i> LOGOUT
                                </div>
                        ...
                        <div class="col-12">
                            <div class="p-3 border rounded bg-light">
                                <div class="small text-muted fw-bold mb-1"><i class="fas fa-business-time"></i> MIN
                                    WORKING HOURS</div>
                                <h5 class="fw-bold mb-0 text-primary" id="view_min_hrs">0 Hrs</h5>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            let token = localStorage.getItem('token') || localStorage.getItem('admin_token') || localStorage
                .getItem('emp_token');
            window.currentRulesData = []; // Store fetched data globally for Excel Export

            // Initialize Select2
            $('#branch_selector').select2({
                placeholder: "Search and select companies or branches...",
                allowClear: true,
            ...
            // Fetch Dropdown Data (For Both Add Modal & Filter Bar)
            $.ajax({
            ...
            function loadData() {
                $.ajax({
                    ...
                    success: function(res) {
                        window.currentRulesData = res.data; // Store for Excel
                        let tbody = '';
                        let mobileCards = '';

                        if (res.data.length === 0) {
                            tbody =
                                '<tr><td colspan="7" class="text-center py-4 text-muted fw-bold">No rules configured yet.</td></tr>';
                            mobileCards =
                                '<div class="col-12 text-center py-4 text-muted fw-bold">No rules configured yet.</div>';
                        } else {
                            res.data.forEach(item => {
                                // Location Naming & ID tracking
                            ...
                            <tr class="rule-item" data-rule-id="${item.id}" data-loc-id="${locId}">
                                ...
                                <td><span class="badge bg-light text-danger border border-danger">${item.logout_start.substring(0,5)} - ${item.logout_end.substring(0,5)}</span></td>
                                <td class="fw-bold text-secondary">${item.min_working_hours} Hrs</td>
                                <td><span class="badge ${badgeClass}">${item.status.toUpperCase()}</span></td>
                                <td class="text-center">
                                    <button class="btn btn-sm btn-light border text-info me-1" onclick="viewRule('${safeLocName}', '${item.login_start}', '${item.login_end}', '${item.logout_start}', '${item.logout_end}', '${item.min_working_hours}', '${badgeClass}', '${item.status}')" title="View"><i class="fas fa-eye"></i></button>
                                    <button class="btn btn-sm btn-light border text-primary me-1" onclick="editRule(${item.id}, '${safeLocName}', '${item.login_start}', '${item.login_end}', '${item.logout_start}', '${item.logout_end}', '${item.min_working_hours}')" title="Edit"><i class="fas fa-edit"></i></button>
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="row g-2 mb-3">
                                            ...
                                            <div class="col-6">
                                                ...
                                                <div class="fw-bold text-danger small">${item.logout_start.substring(0,5)} - ${item.logout_end.substring(0,5)}</div>
                                            </div>
                                        </div>
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="badge bg-light text-dark border">Min: ${item.min_working_hours} Hrs</span>
                                            <div>
                                                <button class="btn btn-sm btn-outline-info py-0 px-2 me-1" onclick="viewRule('${safeLocName}', '${item.login_start}', '${item.login_end}', '${item.logout_start}', '${item.logout_end}', '${item.min_working_hours}', '${badgeClass}', '${item.status}')"><i class="fas fa-eye" style="font-size:11px;"></i></button>
                                                <button class="btn btn-sm btn-outline-primary py-0 px-2 me-1" onclick="editRule(${item.id}, '${safeLocName}', '${item.login_start}', '${item.login_end}', '${item.logout_start}', '${item.logout_end}', '${item.min_working_hours}')"><i class="fas fa-edit" style="font-size:11px;"></i></button>
            ...
            // ==========================================
            // EXPORT TO EXCEL LOGIC (Export Filtered Data)
            // ==========================================
            $('#btnExportExcel').click(function() {
                if (window.currentRulesData.length === 0) {
                    Swal.fire('Notice', 'No rules data available to export.', 'info');
                    return;
                }
                ...
                if (visibleIds.length === 0) {
                    Swal.fire('Notice', 'No rules match your filter to export.', 'warning');
                    return;
                }

                let csvContent = "data:text/csv;charset=utf-8,";
                let headers = ["Location Name", "Branch / Type", "Login Window", "Logout Window",
                    "Min Working Hours", "Status"
                ];
            ...
            // ==========================================
            // CRUD ACTIONS (ADD, VIEW, EDIT, DELETE)
            // ==========================================
            ...
            window.viewRule = function(locName, lStart, lEnd, loStart, loEnd, minHrs, badgeClass, statusText) {
            ...
            window.editRule = function(id, locName, lStart, lEnd, loStart, loEnd, minHrs) {
                $('#edit_id').val(id);
