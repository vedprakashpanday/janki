    ...
    <script>
        $(document).ready(function() {
            ...
            let table = $('#ledgerTable').DataTable({
                ...
                columns: [
                    { data: 'ledger_code', render: d => `<span class="fw-bold text-primary">${d}</span>` },
                    { data: 'branch_id', render: (d, t, row) => `<span class="badge bg-light text-dark border">${row.branch ? row.branch.branch_name : 'N/A'}</span>` },
                    { data: 'ledger_name', render: d => `<span class="fw-bold text-dark">${d}</span>` },
                    { data: 'from_date', render: d => d ? d : '-' },
                    { data: 'to_date', render: d => d ? d : '-' },
                    { data: 'status', render: d => d === 'Active' ? `<span class="badge bg-success">Active</span>` : `<span class="badge bg-danger">Inactive</span>` },
                    { 
    data: 'id', 
    orderable: false, 
    ...
    render: d => `
        <div class="d-flex justify-content-end flex-nowrap gap-1">
            ...
            function renderMobileCards(data) {
                let html = '';
                data.forEach(item => {
                    let stBadge = item.status === 'Active' ? `<span class="badge bg-success">Active</span>` : `<span class="badge bg-danger">Inactive</span>`;
            ...
            $(document).on('click', '.view-btn', function() {
                ...
                $.get({
                    ...
                    success: function(res) {
                        ...
                        let badge = d.status === 'Active' ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>';
            ...
            $('#ledgerForm').submit(function(e) {
                e.preventDefault();
                let id = $('#edit_id').val();
                let url = mode === 'add' ? '/api/v1/admin/ledgers' : `/api/v1/admin/ledgers/${id}`;
                ...
                $.ajax({
                    ...
                    complete: function() { btn.prop('disabled', false).text('Save Ledger'); }
                });
            });
            ...
            $(document).on('click', '.delete-btn', function() {
                if (confirm("Are you sure?")) {
                    $.ajax({
                        url: `/api/v1/admin/ledgers/${$(this).data('id')}`,
                        type: 'DELETE',
                        headers: { 'Authorization': 'Bearer ' + apiToken },
