        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex gap-2">
                <button type="button" class="btn text-white px-3 py-2 shadow-sm secured-item"
                    data-permission="general_leads_add_direct" style="background-color: #10b981;"
            ...
            <div class="card-body">
                <form id="assignForm" class="row g-3 align-items-end">
                    <div class="col-md-4"><label class="form-label">Telecaller ID / Name</label><input type="text"
                            id="a_telecaller" class="form-control" list="staffDataList" required autocomplete="off"></div>
                    ...
                    <div class="col-md-2"><button type="submit" class="btn text-white w-100 fw-medium"
                            style="background-color: var(--sidebar-bg);">Assign Data</button></div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body p-4">
                    <form id="custForm" class="row g-3">
                        ...
                        <div class="col-md-3"><label class="form-label">Status</label><input type="text"
                                name="status" id="f_status" class="form-control" value="General" readonly
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
           ...
           // Apni script mein jahan ye function hai usko isse replace karo:
        ...
        error: function() {
            ...
            function loadAllData() {
                if (!$.fn.DataTable.isDataTable('#dataTableMain')) {
                  ...
                  // DataTable initialization ke andar buttons property ko ye do:
buttons: [{
    ...
    action: function(e, dt, node, config) {
        ...
        let btn = $(node);
        let originalText = btn.html();
        btn.html('<i class="fas fa-spinner fa-spin me-1"></i> Exporting...'); // Loading effect
        
        $.ajax({
            url: '/api/v1/general-leads/export?search=' + encodeURIComponent(searchVal),
            type: 'GET',
            success: function(res) {
                btn.html(originalText);
                if(res.data && res.data.length > 0) {
                    // 🔥 FIX 3: Asli .xlsx generation using SheetJS
                    let ws = XLSX.utils.json_to_sheet(res.data);
                    let wb = XLSX.utils.book_new();
                    XLSX.utils.book_append_sheet(wb, ws, "General Leads");
                    XLSX.writeFile(wb, "General_Leads_Export.xlsx"); // .xlsx format
                } else {
            ...
            error: function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        pendingData = (res.pending_requests || []).filter(d => d.status
                        .toLowerCase() === 'general');
            ...
            function renderMobile() {
                ...
                chunk.forEach(d => {
                    $('#mContainer').append(
                        `<div class="mobile-item m-card-item"><div class="d-flex justify-content-between"><div><h6 class="fw-bold">${d.cust_name}</h6></div><span class="badge bg-secondary">${d.status}</span></div><div class="small text-muted mb-2">${d.mobile} | Tele: ${d.assigned_telecaller||'N/A'}</div><div class="d-flex gap-2 border-top pt-2 mt-2"><button class="btn btn-sm btn-light text-primary flex-fill edit-btn secured-item" data-permission="general_leads_edit" data-id="${d.id}"><i class="fas fa-edit"></i> Edit</button><button class="btn btn-sm btn-light text-danger flex-fill delete-btn secured-item" data-permission="general_leads_delete" data-id="${d.id}"><i class="fas fa-trash"></i> Delete</button></div></div>`
                        );
                });
                mIndex += CHUNK;
                ...
                reader.onload = function(event) {
                    ...
                    rawData.forEach(row => {
                        ...
                        allLeads.push({
                            ...
                            "status": row[13] ?? "General",
                            "followup_date": (row[14] && row[14] !== "-") ? row[14] :
                                null,
                    ...
                    async function uploadChunks() {
                        for (let i = 0; i < totalChunks; i++) {
                            ...
                            try {
                                ...
                                alert("Import Error at chunk " + (i + 1));
                                return;
                            }
                        }
                        alert("All " + allLeads.length + " leads imported successfully!");
                        loadAllData();
                        $('#importExcel').val('');
                    }
                    uploadChunks();
